...
* **Metodi Getter e Setter**: forniscono un controllo maggiore sull'accesso alla modifica dei dati. Permettono di ottenere e accedere un attributo in una data classe (get) e modificare (set) gli attributi privati di una classe.



**Livello di visibilità** (o scope delle variabili):

* **Globale**: conosciuta e dichiarata fuori dalle funzioni, accessibile da qualsiasi punto del codice. A meno che non venga oscurata da una variabile locale. Keyword "global".
* **Locale**: le variabili locali sono dichiarate all'interno della funione e sono accessibili all'interno di quella funzione.
* **Non-locale**: si applica nelle funzioni annidate. Accessibile solo nella funzione interna alla classe. Keyword non-local.
